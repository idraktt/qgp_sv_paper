...
plot.totalCounts <- function(dat, svtypes, thousandG=F){
  ...
  mtext(2, text="SVs Discovered (log10)", line=2)
...
plot.HWE <- function(dat, pop=NULL, title=NULL, full.legend=F, lab.cex=1){
  ...
  HW.cols <- rep("#59A14F", times=length(HW.p))
  HW.cols[which(HW.p<0.05)] <- "#8CD17D"
  HW.cols[which(HW.p<0.05/length(HW.p))] <- "#E15759"
  ...
  HWTernaryPlot(X=HWE.mat, n=max(HWE.mat, na.rm=T), newframe=F, 
                ...
                alpha=0.05, 
                curvecols=c("#59A14F", "#8CD17D", NA, NA), pch=NA)
  ...
  HWTernaryPlot(X=HWE.mat, n=max(HWE.mat, na.rm=T), newframe=F, 
                ...
                curvecols=c("#59A14F", "#E15759", NA, NA), 
  ...
  print(paste("BONFERRONI P: ", 0.05/length(HW.p), sep=""))
  
  
  legend("right", pch=19, col=c("#59A14F", "#8CD17D", "#E15759"), pt.cex=1.3, 
...
vcf2bed.in <- "/home/ealiyev_qgp/QF-QBB-RES-ACC-0032/QGP_SV/reports/population_final/QGP_hq.svtk.bed"
...
dat.all$AN<-dat.all$NSAMP
dat$AN<-dat$NSAMP
dat$AC<-dat$QGP_ADM_AC + dat$QGP_AFR_AC + dat$QGP_WEP_AC + dat$QGP_GAR_AC + dat$QGP_PAR_AC + dat$QGP_SAS_AC
dat.all$AC<-dat.all$QGP_ADM_AC + dat.all$QGP_AFR_AC + dat.all$QGP_WEP_AC + dat.all$QGP_GAR_AC + dat.all$QGP_PAR_AC + dat.all$QGP_SAS_AC
dat$SVLEN<-dat$end - dat$start
dat.all$SVLEN<-dat.all$end - dat.all$start
dat<-dat[which(dat$AN>0),]
dat.all<-dat.all[which(dat.all$AN>0),]

dat$N_BI_GENOS<-6047
dat$QGP_ADM_N_BI_GENOS<-1180
dat$QGP_AFR_N_BI_GENOS<-92
dat$QGP_GAR_N_BI_GENOS<-2311
dat$QGP_PAR_N_BI_GENOS<-1052
dat$QGP_SAS_N_BI_GENOS<-38
dat$QGP_WEP_N_BI_GENOS<-1372
...
plot.freqByType(dat=dat[which(is.na(dat$LOF)
                              ...
                              | !is.na(dat$DUP_PARTIAL)), ], 
                ymin=0.38, axlabel.cex=0.8)
plot.freqByType(dat=dat[which(dat$Intronic=="True"), ], 
                ymin=0.38, axlabel.cex=0.8)
dev.off()
...
plot.fracSingletons.bySize(dat=dat, 
                           svtype="ALL", include.ins=F, 
                           d=c(-1, 1000, 10000, 100000, 1000000, 1000000000), 
                           d.labels=c("<1kb", "1-10kb", "10-100kb", "100kb-1Mb", ">1Mb"), 
                           mar=c(4, 2.75, 0.5, 1), ylab.cex=1)
p.sing.dat<-calc.fracSingletons.singleClass.binned(dat=dat,"ALL",include.ins = F,d=c(-1, 1000, 10000, 100000, 1000000, 1000000000),aps = F)
